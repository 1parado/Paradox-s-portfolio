...
function Row({ field }: { field: Field }) {
  return (
    <div className="flex items-baseline gap-3 rounded-2xl border border-white/10 bg-white/5 px-4 py-3">
      <span className="w-14 shrink-0 text-xs font-semibold uppercase tracking-[0.18em] text-sky-300/70">{field.label}</span>
      {field.href ? (
        <a
          href={field.href}
...
export function AboutCard({ children }: { children?: ReactNode }) {
  return (
    <div className="flex h-full flex-col rounded-[1.5rem] border border-white/10 bg-[radial-gradient(circle_at_15%_10%,rgba(56,189,248,0.20),transparent_34%),linear-gradient(145deg,rgba(15,23,42,0.96),rgba(3,7,18,0.92))] p-6 text-white shadow-inner">
      <div>
        <div className="inline-flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-sky-400 to-indigo-600 text-2xl text-white shadow-lg shadow-sky-500/20">
          👋
        </div>
        <p className="mt-6 text-xs font-semibold uppercase tracking-[0.22em] text-sky-200/70">About</p>
        <h2 className="mt-2 text-3xl font-semibold tracking-tight">关于我</h2>
      </div>
