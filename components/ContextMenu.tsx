...
export function ContextMenu({ app, pages, onClose, onOpen, onOpenExternal, editing, onRequestEdit, onMove, onDelete, onRename }: Props) {
  return (
    <div className="absolute inset-0 z-40 flex items-end bg-black/40 p-4 md:items-center md:justify-center" onClick={onClose}>
      <div className="w-full max-w-sm rounded-[2rem] border border-white/10 bg-zinc-950/95 p-4 text-white shadow-2xl" onClick={(event) => event.stopPropagation()}>
        ...
        <div className="grid gap-2">
          ...
          {editing ? (
            <>
              {app.type === 'folder' && onRename ? (
                <button type="button" className="rounded-2xl bg-white/10 px-4 py-3 text-left" onClick={onRename}>重命名</button>
              ) : null}
              {pages.map((page) => (
